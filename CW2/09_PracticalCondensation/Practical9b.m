...
%
% Then TO DO:
% - 1.Try varying the number of particles: 2000, 500, 100,...
% - 2.Change the state to have 2 more degrees of freedom: velX and velY.
%add two dims=1/x,1/y？
%   (This will require you to change how state-predictions are made, and 
%   how they are converted to measurement-space)
% - 3.Visualize the top-scoring particles (more than just 1!)
...
%?TO DO 2?
%Assume the x, y velocity are uniform
%particles_old(:,3)=ones( numParticles, 1)*13;%velx
%particles_old(:,4)=ones( numParticles, 1)*9;%vely
%?TO DO 2?
% ============================
...
%TO DO 3 htop shows the top-scoring particles
htop=figure; %figure 3
...
for( iTime = 1:22 )
    ...
    for (particleNum = 1:numParticles)
        ...
        %?TO DO 2?
        %%the new particles: particles_new=particles_old+noise+velocity.
        %%After adding the velx, vely, the particles will be more closer to
        %%the tracking object.
        %noise = 10*randn(1,2);
        %particles_new(particleNum,:) = particles_old( samples_to_propagate(particleNum),: ) ;
        %particles_new(particleNum,1:2)=particles_new(particleNum,1:2)+noise+particles_new(particleNum,3:4);
        %particles_new(particleNum,:) = round(  particles_new(particleNum,:)  ); % Round the particles_new to simplify Likelihood evaluation.
        %?TO DO 2?
    ...
    %?________?
    %adding two constants: 9,13 to compensate for the influence of velocity and the position of matching point
    %particles_new(:,2)=particles_new(:,2)+9;
    %particles_new(:,1)=particles_new(:,1)+13;
    %?________?
    ...
    %?TO DO 3? If the weight of a particle is not equals zero, this
    %particle is top-scoring. Get the top-scoring particles and visualize
    %them--figure 3
    topnum=numParticles-sum(weight_of_samples==0);
    set(0,'CurrentFigure',htop)
    set(gcf,'Position',[23 125 640 480]);
    set(gcf,'Color',[1 1 1]);            
    topparticles=sortrows([weight_of_samples particles_new],-1);
    topparticles=topparticles(1:topnum,2:3);
    plot(topparticles(:,2), topparticles(:,1), 'r.','MarkerSize',5)
    title(sprintf( 'Top-scoring Particles\n(Time %d)', iTime));
    xlim([0 imgWidth]);
    ylim([0 imgHeight]);
    drawnow
    %?TO DO 3?
