function r=practical9candedgedetector( templateMetaFileName )
...
for( iTime = 1:numFrames )
    ...
    si = size(Im2);
    edgepixels = edgedetector(Im2);
      %  particlesindex=(particles_new(:,2)-1)*s(1)+particles_new(:,1);
    edgeindex=find(edgepixels>0);
      %  particlesindex=ismember(particlesindex,edgeindex);
    ...
    for particleNum = 1:numParticles
        ...
        % the likelihood.
        particleindex_extra = (particle(2)-1)*si(1)+particle(1);
    ...
    %?Extra?
    particlesindex=(particles_new(:,2)-1)*si(1)+particles_new(:,1);
    particlesonedge=(ismember(particlesindex,edgeindex)==0)*1/2+ismember(particlesindex,edgeindex);
    weight_of_samples=weight_of_samples.*particlesonedge;

    
    set(0,'CurrentFigure',hedgeimg)
    colormap(gray);
    set(gcf,'Position',[23 125 640 480]);
    set(gcf,'Color',[1 1 1]);
    title(sprintf( 'Edge detector\n(Time %d)', iTime));
    hold on
    imagesc(edgepixels)
    hold off
    drawnow
    
   %weight_of_samples=weight_of_samples.*particlesindex;
